...
if (_mode == "groups") exitWith {
    hint "Empty Groups favorites are disabled for now";
    [ZEN_FILTER_LOG_LEVEL_WARN, "Empty Groups favorite toggle skipped: group/composition placement needs a separate implementation"] call zen_filter_main_fnc_log;
};
...
private _favoriteEntry = [_sourceDisplayPath, _className, _favoriteId];
...
if (_existingIndex == -1) then {
    ...
    [ZEN_FILTER_LOG_LEVEL_INFO, format [
        "added Empty favorite mode=%1 class=%2 id=%3 displayPath=%4",
        _mode,
        _className,
        _favoriteId,
        _sourceDisplayPath
    ...
    [ZEN_FILTER_LOG_LEVEL_INFO, format [
        "removed Empty favorite mode=%1 class=%2 displayPath=%3",
        _mode,
        _className,
        _removed select 0
    ]] call zen_filter_main_fnc_log;
